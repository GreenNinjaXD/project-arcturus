<template>
    <div class="wrapper" id="app">
        <div class="player">
            <div class="tbt">
                <div class="player__top">
                    <div class="player-cover">
                        <img src="@/assets/img/moonlight.png" alt="">
                    </div>
                    <div class="tbh">
                        ...
                        <div class="player-controls">
                            <div class="player-controls__item -favorite" :class="{ active: currentTrack.favorited }"
                                @click="favorite">
                                <svg class="icon heartheh">
                                    <use xlink:href="#icon-heart-o"></use>
                                </svg>
                            </div>
                            <div class="main-control">
                                <div class="player-controls__item main-control-prev">
                                    <svg class="icon">
                                        <use xlink:href="#icon-prev"></use>
                                    </svg>
                                </div>
                                <div class="player-controls__item -xl js-play main-control-play" @click="play()">
                                    <svg class="icon">
                                        <use xlink:href="#icon-pause" v-if="isTimerPlaying"></use>
                                        <use xlink:href="#icon-play" v-else></use>
                                    </svg>
                                </div>
                                <div class="player-controls__item main-control-next">
                                    <svg class="icon">
                                        <use xlink:href="#icon-next"></use>
                                    </svg>
                                </div>
                            </div>
                            <div class="list__opener">
                                <div class="huh" :class="{ heh: drActive }" @click="drActive = !drActive">
                                    <i class="fa-solid fa-horizontal-rule znd fst"></i>
                                </div>
                                <div class="huh-t" :class="{ heh: drActive }" @click="drActive = !drActive">
                                    <i class="fa-solid fa-horizontal-rule znd disap"></i>
                                </div>
                                <div class="huh-tt" :class="{ heh: drActive }" @click="drActive = !drActive">
                                    <i class="fa-solid fa-horizontal-rule znd scnd"></i>
                                </div>
                            </div>
                        </div>
                    </div>
            ...
            <video src="../../assets/video/music-1.mp4" class="player__video" muted autoplay repeat></video>
            <transition name="msclst-fade">
                <div class="music-list" v-if="drActive">
                    <div class="info-about">
                        <div class="info-now">
                            <div class="info-jcsb" :class="{ tractive: isTimerPlaying }">
                                <span class="info-about-name">{{ currentTrack.artist }}</span>
                                <span class="info-about-track">{{ currentTrack.name }}</span>
                            </div>
                            <div class="info-time">
                                <span class="info-time-dur">{{ isTimerPlaying ? 'Playing' : duration }}</span>
                            </div>
                        </div>
                        <div class="info-ex">
                            <div class="info-jcsb">
                                <span class="info-about-name">Kali Uchis</span>
                                <span class="info-about-track">moonlight</span>
                            </div>
                            <div class="info-time">
                                <span class="info-time-dur">03:49</span>
                            </div>
                        </div>
                        <div class="info-ex">
                            <div class="info-jcsb">
                                <span class="info-about-name">Marvel</span>
                                <span class="info-about-track">Ultimate Spider Man Theme</span>
                            </div>
                            <div class="info-time">
                                <span class="info-time-dur">01:26</span>
                            </div>
                        </div>
                        <div class="info-ex">
                            <div class="info-jcsb">
                                <span class="info-about-name">Metro Boomin</span>
                                <span class="info-about-track">Annihilate</span>
                            </div>
                            <div class="info-time">
                                <span class="info-time-dur">02:28</span>
                            </div>
                        </div>
                        <div class="info-ex">
                            <div class="info-jcsb">
                                <span class="info-about-name">Post Malone</span>
                                <span class="info-about-track">Sunflower</span>
                            </div>
                            <div class="info-time">
                                <span class="info-time-dur">02:41</span>
                            </div>
                        </div>
                    </div>
                </div>
...
import layout from '@/components/Dial/Dial.vue';
...
function generateTime() {
    ...
    const width = (100 / audio.value.duration) * audio.value.currentTime;
    barWidth.value = width + '%';
    circleLeft.value = width + '%';

    const durmin = Math.floor(audio.value.duration / 60);
    const dursec = Math.floor(audio.value.duration - durmin * 60);
    const curmin = Math.floor(audio.value.currentTime / 60);
    const cursec = Math.floor(audio.value.currentTime - curmin * 60);

    duration.value = `${durmin.toString().padStart(2, '0')}:${dursec.toString().padStart(2, '0')}`;
    currentTime.value = `${curmin.toString().padStart(2, '0')}:${cursec.toString().padStart(2, '0')}`;
}


function favorite() {
    tracks.value[currentTrackIndex.value].favorited = !tracks.value[currentTrackIndex.value].favorited;
}
